						...
						float laplacian, float num, float den, float std_dev, float std_dev2, float *diff_coef_device)
{
	...
	if(i > 0 && i < height-1 && j > 0 && j < width-1) {
		...
		north_deriv_device[k] = image_device[(i - 1) * width + j] - image_device[k];	// north derivative --- 1 floating point arithmetic operations
		south_deriv_device[k] = image_device[(i + 1) * width + j] - image_device[k];	// south derivative --- 1 floating point arithmetic operations
		west_deriv_device[k] = image_device[i * width + (j - 1)] - image_device[k];	// west derivative --- 1 floating point arithmetic operations
		east_deriv_device[k] = image_device[i * width + (j + 1)] - image_device[k];	// east derivative --- 1 floating point arithmetic operations
		gradient_square = (north_deriv_device[k] * north_deriv_device[k] + south_deriv_device[k] * south_deriv_device[k] + west_deriv_device[k] * west_deriv_device[k] + east_deriv_device[k] * east_deriv_device[k]) / (image_device[k] * image_device[k]); // 9 floating point arithmetic operations
		laplacian = (north_deriv_device[k] + south_deriv_device[k] + west_deriv_device[k] + east_deriv_device[k]) / image_device[k]; // 4 floating point arithmetic operations
		...
		diff_coef_device[k] = 1.0 / (1.0 + den); // 2 floating point arithmetic operations
		if (diff_coef_device[k] < 0) {
			diff_coef_device[k] = 0;
		} else if (diff_coef_device[k] > 1)	{
...
{	
	...
	for (int iter = 0; iter < n_iter; iter++) {
		...
		//reduction<<<grid, threads>>>(image_device, sum);
